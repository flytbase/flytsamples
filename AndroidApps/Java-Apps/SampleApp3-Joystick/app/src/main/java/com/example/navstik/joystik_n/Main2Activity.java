...
public class Main2Activity extends AppCompatActivity {
        ...
        public boolean onTouch(View v, MotionEvent event) {
            switch(event.getAction()) {
                case MotionEvent.ACTION_DOWN:
                    // PRESSED
                    VelocitySet(0,-2,0,0); // if you want to handle the touch event
                    return true;
    ...
    private View.OnTouchListener buttonFrontListener = new View.OnTouchListener() {
        ...
        public boolean onTouch(View v, MotionEvent event) {
            switch(event.getAction()) {
                case MotionEvent.ACTION_DOWN:
                    // PRESSED
                    VelocitySet(2,0,0,0); // if you want to handle the touch event
                    return true;
                case MotionEvent.ACTION_UP:
                    // RELEASED
                    VelocitySet(0,0,0,0); // if you want to handle the touch event
    ...
    private View.OnTouchListener buttonUpListener = new View.OnTouchListener() {
        ...
        public boolean onTouch(View v, MotionEvent event) {
            switch(event.getAction()) {
                case MotionEvent.ACTION_DOWN:
                    // PRESSED
                    VelocitySet(0,0,-1,0); // if you want to handle the touch event
                    return true;
                case MotionEvent.ACTION_UP:
                    // RELEASED
                    VelocitySet(0,0,0,0); // if you want to handle the touch event
    ...
    private View.OnTouchListener buttonTurnLeftListener = new View.OnTouchListener() {
        ...
        public boolean onTouch(View v, MotionEvent event) {
            switch(event.getAction()) {
                case MotionEvent.ACTION_DOWN:
                    // PRESSED
                    VelocitySet(0,0,0,-0.5); // if you want to handle the touch event
                    return true;
                case MotionEvent.ACTION_UP:
                    // RELEASED
                    VelocitySet(0,0,0,0); // if you want to handle the touch event
    ...
    private void VelocitySet(final double x, final double y, final double z, final double zang){
        class VelocitySetRequest extends AsyncTask<Void, Void, String> {
            @Override
            protected String doInBackground(Void... params) {
                try {
                    //Rest url
                    final String url = "http://"+ip+":9090/ros/"+namespace+"/navigation/velocity_set";
                    //params in json
...
//                        Toast.makeText(getApplicationContext(), "command sent.", Toast.LENGTH_SHORT).show();
        ...
        new VelocitySetRequest().execute();
