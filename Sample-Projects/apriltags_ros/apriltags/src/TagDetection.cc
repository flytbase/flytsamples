...
void TagDetection::draw(cv::Mat& image, std::string frame_name) const {
  ...
  // plot outline
  cv::line(image, cv::Point2f(p1.first, p1.second), cv::Point2f(p2.first, p2.second), cv::Scalar(0,255,0,0),2 );
  cv::line(image, cv::Point2f(p2.first, p2.second), cv::Point2f(p3.first, p3.second), cv::Scalar(0,255,0,0),2 );
  cv::line(image, cv::Point2f(p3.first, p3.second), cv::Point2f(p4.first, p4.second), cv::Scalar(0,255,0,0),2 );
  cv::line(image, cv::Point2f(p4.first, p4.second), cv::Point2f(p1.first, p1.second), cv::Scalar(0,255,0,0),2 );
 ...
 // cv::circle(image, cv::Point2f(cxy.first, cxy.second), 8, cv::Scalar(0,0,255,0), 2);
  ...
  cv::putText(image, strSt.str(),
              cv::Point2f(p1.first - 50, p1.second + 14),
              cv::FONT_HERSHEY_TRIPLEX, 0.6, cv::Scalar(0,0,255),1.8);
}
