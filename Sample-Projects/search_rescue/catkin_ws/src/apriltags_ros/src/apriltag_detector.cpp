...
_takeoff_longitude = 0;
 ...
 // AprilTags::TagCodes tag_codes = AprilTags::tagCodes36h11;
  ...
  _latlong_sub = nh.subscribe("/"+global_namespace+"/mavros/global_position/global",1, &AprilTagDetector::gposCb, this);
  _camp_site_pub = nh.advertise<AprilTagCampSite>("camp_site_coordinates",1);
    _pos_set_result_sub = nh.subscribe("/"+global_namespace+"/navigation_server_pos/result",1,&AprilTagDetector::posSetResultCb,this);

  lposdata_sub_ = nh.subscribe("/"+global_namespace+"/mavros/local_position/local",1,&AprilTagDetector::lposdataCb,this);
    imudata_sub_ = nh.subscribe("/"+global_namespace+"/mavros/imu/data_euler", 1, &AprilTagDetector::imudataCb, this);
    lidar_sub_ = nh.subscribe("/"+global_namespace+"/mavros/distance_sensor/lidarlite_pub", 1, &AprilTagDetector::lidarCb, this);
}
...
void AprilTagDetector::posSetResultCb(const navigation_util::NavigationPosActionResultConstPtr &pos_set_result)
{
  if(pos_set_result->status.status == pos_set_result->status.SUCCEEDED && (pos_set_result->result.pose.header.frame_id == "takeoff" || pos_set_result->result.pose.header.frame_id == "takeoff_rostful"))
    {
      //takeoff detected
      _takeoff_latitude = _last_latitude;
      _takeoff_longitude = _last_longitude;
    }
}

//////////////////////////////////////////
void AprilTagDetector::imudataCb(const geometry_msgs::TwistStampedConstPtr &imu_msg)
{
    imu_msg_data_ptr = imu_msg;
    //TODO: Copy imu_msg to data member (imu sg type) of the class and in image callback copy from data member to local loop variables
}

void AprilTagDetector::lposdataCb(const geometry_msgs::TwistStampedConstPtr &lpos_msg)
{
    lpos_msg_data_ptr = lpos_msg;
    //TODO: Copy imu_msg to data member (imu sg type) of the class and in image callback copy from data member to local loop variables
}

void AprilTagDetector::lidarCb(const sensor_msgs::RangeConstPtr &lidar)
{
    lidar_ptr = lidar;
}
...
void AprilTagDetector::imageCb(const sensor_msgs::ImageConstPtr& msg,const sensor_msgs::CameraInfoConstPtr& cam_info){
  ...
  ///////////////////////////////////////////////
  float roll, pitch, sonarDist;
  float navXInc, navYInc, navXIcomp, navYIcomp, navXI, navYI, navXGnc, navYGnc, navXGcomp, navYGcomp, navXG, navYG, navXtr, navYtr;

  if(lidar_ptr)
      sonarDist = lidar_ptr->range;//-lpos_msg_data_ptr->twist.linear.z;;
  else if(lpos_msg_data_ptr)
      sonarDist = -lpos_msg_data_ptr->twist.linear.z;
  else
      sonarDist = 3.0;
  if(imu_msg_data_ptr != NULL)
  {
    roll = imu_msg_data_ptr->twist.linear.x;
    pitch = imu_msg_data_ptr->twist.linear.y;
  }
  ...
  BOOST_FOREACH(AprilTags::TagDetection detection, detections){
    ...
    navXIcomp = -tanf(roll)*sonarDist;
    navYIcomp = tanf(pitch)*sonarDist;
  

    ///////////////////////////////////////////////////
    camp_site.x_position = tag_pose.pose.position.x + navXIcomp;
    camp_site.y_position = tag_pose.pose.position.y + navYIcomp;
    camp_site.latitude =  _last_latitude  - _takeoff_latitude;
    camp_site.longitude = _last_longitude - _takeoff_longitude;
